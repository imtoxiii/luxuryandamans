...
const Header = () => {
  ...
  return (
    <>
      {/* Fixed Navigation */}
      <motion.nav
        className={`fixed top-0 left-0 right-0 w-full z-[10001] transition-all duration-500 ${(showWhiteBackground && !isMenuOpen) ? 'nav-white-bg shadow-lg' : 'bg-transparent'
          }`}
        style={{
          zIndex: 10001,
          position: 'fixed',
        ...
        <div className="relative container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <Link to="/" className="flex items-center space-x-2 group" onClick={closeMenu}>
              ...
              <span className={`text-2xl font-display font-bold transition-colors duration-300 ${(showWhiteBackground && !isMenuOpen) ? 'text-gray-900' : 'text-white'
                }`}>
                Luxury Andamans
              </span>
            </Link>
            ...
            <div className="hidden md:flex items-center lg:space-x-8 md:space-x-4">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.path}
                  className={`text-sm font-medium transition-all duration-300 hover:scale-105 ${((item.name === 'Calculator') ? 'hidden lg:inline-block' : '')
            ...
            >
              ...
              <motion.span
                className={`absolute w-8 h-1 rounded-full shadow-2xl ${isMenuOpen ? 'bg-white' : (showWhiteBackground ? 'bg-gray-900' : 'bg-white')
                  }`}
                initial={false}
                animate={{
