...
const NumberInput = ({ value, onChange, min, max, unit }: { value: number, onChange: (val: number) => void, min: number, max: number, unit: string }) => (
    <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-xl p-2 w-full">
        <button onClick={() => onChange(Math.max(min, value - 1))} className="p-3 rounded-lg bg-white shadow-sm hover:bg-gray-100 transition text-gray-600 disabled:opacity-50 disabled:cursor-not-allowed" disabled={value <= min}>
            <Minus className="w-4 h-4" />
        </button>
        <span className="font-bold text-lg text-gray-800">{value} {unit}{value > 1 ? 's' : ''}</span>
        <button onClick={() => onChange(Math.min(max, value + 1))} className="p-3 rounded-lg bg-white shadow-sm hover:bg-gray-100 transition text-gray-600 disabled:opacity-50 disabled:cursor-not-allowed" disabled={value >= max}>
...
const EnquiryForm = ({ planDetails, onBack, onSubmitSuccess }: { planDetails: any, onBack: () => void, onSubmitSuccess: () => void }) => {
    ...
    return (
        <div className="absolute inset-0 w-full h-full bg-white rounded-3xl p-6 md:p-8 flex flex-col shadow-2xl border border-gray-100 overflow-hidden" style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
            {/* Decorative Background */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-blue-50 rounded-full -translate-y-32 translate-x-32 blur-3xl opacity-50 pointer-events-none"></div>
            <div className="absolute bottom-0 left-0 w-64 h-64 bg-cyan-50 rounded-full translate-y-32 -translate-x-32 blur-3xl opacity-50 pointer-events-none"></div>

            <div className="relative z-10 flex items-center justify-between mb-4 md:mb-6 flex-shrink-0">
                <button onClick={onBack} className="flex items-center text-sm font-medium text-gray-500 hover:text-blue-600 transition-colors group px-3 py-2 rounded-lg hover:bg-gray-50">
            ...
            <div className="flex-grow overflow-y-auto custom-scrollbar -mx-4 px-4 pb-2">
                <div className="relative z-10 text-center mb-6 p-4 md:p-6 bg-gradient-to-br from-gray-50 to-white rounded-2xl border border-gray-100 shadow-sm">
                    <p className="text-3xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-cyan-600 mb-2 font-display">{formatPrice(planDetails.totalCost)}</p>
                    <p className="text-sm text-gray-500 font-medium">Your personalized trip estimate</p>
                </div>

                <form onSubmit={handleSubmit} className="relative z-10 flex flex-col space-y-4 md:space-y-5">
                    <input type="hidden" name="form_type" value="pricing_calculator" />
                    <div className="space-y-4 md:space-y-5">
                        <div className="group">
                            <label htmlFor="name" className="text-sm font-semibold text-gray-700 mb-1.5 block flex items-center">
                            ...
                            <input 
                                ...
                                className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all bg-gray-50 focus:bg-white shadow-sm text-base" 
                                placeholder="John Doe" 
                            />
                        </div>
                        <div className="group">
                            <label htmlFor="email" className="text-sm font-semibold text-gray-700 mb-1.5 block flex items-center">
                        ...
                        <div className="group">
                            <label htmlFor="phone" className="text-sm font-semibold text-gray-700 mb-1.5 block flex items-center">
                                <Phone className="w-4 h-4 mr-2 text-blue-500" />
                                Phone Number (Optional)
                            </label>
                            <div className="flex">
                                <span className="inline-flex items-center px-4 text-sm font-medium text-gray-600 bg-gray-100 border border-r-0 border-gray-200 rounded-l-xl">+91</span>
                                <input 
                                    type="tel" 
                                    id="phone" 
                                    value={phone} 
                                    onChange={e => setPhone(e.target.value)} 
                                    className="w-full px-4 py-3 border border-gray-200 rounded-r-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all bg-gray-50 focus:bg-white shadow-sm text-base" 
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-xl font-bold text-lg shadow-lg shadow-blue-500/30 hover:shadow-xl hover:scale-[1.02] transition-all group relative disabled:opacity-70 disabled:cursor-not-allowed overflow-hidden mt-2"
                    >
                        {isSubmitting && (
...
const PricingCalculatorPage = () => {
    ...
    return (
        <div className="min-h-screen bg-white font-sans">
            ...
            {/* Hero Section - Redesigned to match Home */}
                ...
                <motion.div variants={staggerContainer(0.1, 0.1)} initial="initial" animate="animate">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                        ...
                        <motion.div variants={fadeInUp} className="lg:col-span-7 bg-white p-6 md:p-8 rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100">
                            ...
                            {/* Navigation Buttons for Mobile/Convenience */}
                        ...
                        <motion.div variants={fadeInUp} className="lg:col-span-5 sticky top-28" style={{ perspective: '1000px' }}>
                            <motion.div
                                className="relative w-full min-h-[550px] md:min-h-[600px]"
                                animate={{ rotateY: isFlipped ? 180 : 0 }}
                                transition={{ duration: 0.8, type: "spring", stiffness: 60, damping: 12 }}
                                style={{ transformStyle: 'preserve-3d' }}
                            >
                                {/* Front of Card - Summary */}
                                <div className="absolute inset-0 w-full h-full bg-white p-6 md:p-8 rounded-3xl shadow-2xl shadow-blue-900/5 border border-gray-100 flex flex-col" style={{ backfaceVisibility: 'hidden' }}>
                                    <AnimatePresence>
                                        {showSuccess && (
                                            <motion.div
                                                ...
                                                className="absolute inset-0 bg-white/90 backdrop-blur-sm flex flex-col items-center justify-center rounded-3xl z-20"
                                            >
                                    ...
                                    <div className="text-center mb-5 p-2 bg-gradient-to-br from-gray-900 to-gray-800 rounded-3xl text-white relative shadow-lg">
                                        <div className="absolute top-0 right-0 w-40 h-40 bg-blue-500/20 rounded-full blur-3xl -mr-20 -mt-20"></div>
                                        <div className="absolute bottom-0 left-0 w-40 h-40 bg-cyan-500/20 rounded-full blur-3xl -ml-20 -mb-20"></div>
                                        <p className="text-4xl md:text-5xl font-bold mb-2 relative z-10 font-display tracking-tight">{formatPrice(totalCost)}</p>
                                    </div>

                                    {totalCost > 0 && (
                                        <div className="flex-grow overflow-hidden flex flex-col">
                                            <h3 className="font-bold text-gray-800 mb-4 text-xs uppercase tracking-widest">Cost Breakdown</h3>
                                            <div className="space-y-4 mb-6 flex-grow flex flex-col overflow-hidden">
                                                <div className="w-full bg-gray-100 rounded-full h-4 overflow-hidden flex flex-shrink-0">
                                                ...
                                                <div className="grid grid-cols-1 gap-3 text-sm overflow-y-auto pr-2 custom-scrollbar flex-grow">
                                                    {breakdown.map((item, index) => (
                                                        <div key={index} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                                                            <div className="flex items-center">
                                                                <div className={`w-3 h-3 rounded-full mr-3 ${['bg-blue-500', 'bg-cyan-500', 'bg-teal-500', 'bg-indigo-500', 'bg-violet-500', 'bg-purple-500', 'bg-fuchsia-500', 'bg-pink-500'][index % 8]}`}></div>
                                                                <span className="text-gray-600 font-medium">{item.category}</span>
                                                            </div>
                                                            <span className="font-bold text-gray-800">{formatPrice(item.amount)}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
